...
double IntTrap(double (*F)(double), double x0,double x1,int N)
	...
	for(int i=0 ; i<N ;i=i+1)
		{
		double area = (F(x0+i*D)+F(x0+((double)i+(double)1)*D))*D/2;
		somma+=area;
		}
	if(sign==1)somma=-somma;
	return (somma);
...
double IntTrap_I(double (*F)(double), double x0,double h,int N)
	...
	for(int i=0 ; i<N ;i=i+1)
		{
		double area = (F(x0+i*h)+F(x0+(i+1.)*h))*h/2;
		somma+=area;
		}
	return (somma);
	}
...
double IntSimp_I(double (*F)(double), double x0,double h,int N)
	...
	for(int i=0 ; i<N ;i=i+1)
		{
		double area = (F(x0+2*i*h)+4*F(x0+(2*i+1)*h)+F(x0+2*(i+1)*h))*(h/3);
		somma+=area;
		}
	return (somma);
	}
