...
ofstream myfile1("1_spaziofasi.txt",ios::trunc);
	for(int i=0;i<N;i++){myfile1<<Th[i]<<"\t\t\t"<<w[i]<<endl;}
	myfile1.close();

	
	t[0]=0;		//valore iniziale
    	Th[0]=(M_PI)*0.25+0.0198;
	w[0] =0;
Runge_Kutta_secondo(F,t,Th,w,dt,N);
	//normalizzazione a pigreco
	for(int i=0;i<N;i++){
	...
	ofstream myfile2("2_spaziofasi.txt",ios::trunc);
...
Runge_Kutta_secondo(F,t,Th,w,dt,N);
	...
	ofstream myfile3("3_spaziofasi.txt",ios::trunc);
