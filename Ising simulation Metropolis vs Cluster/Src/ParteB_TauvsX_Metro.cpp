/*
STUDIO TEMPI DI AUTOCORRELAZIONE PER L'ALGORITMO METROPOLIS
...
int main()
{
	...
	int Nmarkov=50000;					//numero di elementi della catana di markov su cui produco la statistica
	...
	while (k < n_dots_1){	
		...
		X[k] = x;
		sistema.evol_montecarlo(beta, 10000);
	...
	while (k < n_dots_1 + n_dots_2){	
		...
		cout<<"\n>>>>> "<<k<<"-sima Raccolta tau su "<<n_dots_tot<<"(beta = "<<beta<<" )"<<endl;
		X[k] = x;
		sistema.evol_montecarlo(beta, 15000);
		sistema.Raccolta_H_Metropolis( beta, O, Nmarkov);
		Y[k]=Tau(O, Nmarkov, t_max);
		k++;
	}
	...
	string nomeFile= "Output/ParteB/Tau/Metro_TauHvsX.dat";
