/*
STUDIO DISTRIBUZIONE DEI VALORI CAMPIONATI DI <H> A DIVERSI VALORI DI BETA CON L'ALGORITMO A MULTICLUSTER
	produce 5 istogrammi che danno la distribuzio dei valori di H  P vs H
...
int main()
{
	...
	double Omin=-2;						//valore minimo dell'osservabile
	double Omax= 0.05;						//valore max
	...
	while (n < n_graf_1){															
		...
		cout<<"\n>>>>>["<<n<<"] Costruzione Distribuzione H a beta = "<<beta<<endl;
		sistema.evol_SwenWang(beta, 30);
		sistema.Raccolta_H_Cluster( beta, O, Nmarkov);
	
		stringstream sstm;
		sstm << "Output/ParteA/DistribuzioneValori/"<<"distroH_"<< n <<".dat";
