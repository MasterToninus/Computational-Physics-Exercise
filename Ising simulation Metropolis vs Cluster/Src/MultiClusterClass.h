...
int microstato::testbond_up(int i, int j,double beta){
	int test=0;
	int i_up=i+1;
...
void microstato::step_SwenWang(double beta){	
	...
	for(int i=0; i<N; i++){for(int j=0; j<N; j++){ 						//Inizializzazione
		label[i][j]=N*i+j;												//All'inizio ogni nodo è un singolo cluster con il proprio label
		Cluster.push_back(cluster(label[i][j],i,j,N_nodi));				//Per ogni label iniziale si crea una variabile Label
		labeleff.push_back(N*i+j);										//Si crea una lista sequenziale dei label effetivamente utilizzati
	}}
		/*
//-------------------controllo output------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
		*/
//Ciclo dei bond attivati orizzontalmente
	for(int k=0; k<N; k++){for(int l=0; l<N; l++){
		int l_dx=l+1;
		if(l_dx==N)l_dx=0; 

		int labellosalvato= label[k][l_dx]; 			//salvo il labello del nodo dx	
		if(label[k][l]!=labellosalvato){				//verifico che il nodo dx non sia già nel cluster considerato
			if(testbond_dx( k, l, beta)==1){			//verifico se il bond viene attivato
				cluster dummy= Cluster[labellosalvato];
				for(int f=0; f<dummy.N_ele; f++){ 				//ciclo su tutti i nodi del cluster a destra
					label[dummy.i[f]][dummy.j[f]]=label[k][l];	//il label di questi nodi è posto = ai nodi del cluster di sinistra
				}
				...
				for(int i=0; i<labeleff.size(); i++){ 		// questo ciclo è brutto, deve controllare sequenziamente tutto
						if(labeleff[i]== dummy.label){		//si scorre la lista dei label utilizzati  
						labeleff.erase (labeleff.begin()+i);//e si elimina il label del clu dex perchè è unito al cluster clu sin
						break;								
					}
				}
			}
...
//-----------------------------------------------------------------------------------------
		*/
//Ciclo dei bond attivati verticale
	for(int k=0; k<N; k++){for(int l=0; l<N; l++){
		int k_up=k+1;
		if(k_up==N)k_up=0; 

		int labellosalvato= label[k_up][l]; 			//salvo il labello del nodo up	
		if(label[k][l]!=labellosalvato){				//verifico che il nodo up non sia già nel cluster
...
void microstato::step_SwenWang_M_Quad(double beta, double *M_imp, double *M_quad_imp){	//versione a 2 valori, restituisce m improved e m quadrato improved
	int label[N][N];// ad ogni nodo associo il label del cluster a cui appartiene
	vector<cluster> Cluster; // ho un cluster per ogni nodo
	vector<int> labeleff;	//lista variabile dei label effettivamente usati				
...
double microstato::step_SwenWang_Size(double beta){	//versione a 2 valori, restituisce m improved e m quadrato improved
	int label[N][N];// ad ogni nodo associo il label del cluster a cui appartiene
	vector<cluster> Cluster; // ho un cluster per ogni nodo
	vector<int> labeleff;	//lista variabile dei label effettivamente usati				
...
//-------------------------------------------------------------
		*/	
	return((double)sizemedio/(labeleff.size()));
}
