...
double minimo(double *V,int n)//funzione che da il valore minimo di un vettore di double (vale anche per matrici, sono sempre array! pero devo passarle come puntatori
{  
	double dummy = tmax;
	for(int i=0;i<n;i++){if(V[i]<=dummy){dummy=V[i];}}
	return(dummy);
}
...
double Boltzman(int n_int, double *V, int M){
	...
	for(int i=0;i<n_int;i++) if(N[i] != 0){
		double y = (double)N[i]/(M*delta)*sqrt(2*M_PI); 
