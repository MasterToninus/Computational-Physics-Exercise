...
set ylabel "<T>" font "Times-Italic,24" offset 1.5,0 #rotate by 0
#set yrange [0:10]

plot[0:1050] "TvsN.dat" using 1:2:3 notitle "<" w yerrorbars  lt 1 lc 1 lw 2.2

f1(x) = A + B/x		            # define the function to be fit
      A = 1; B = 1;            # initial guess for a1 and b1
fit f1(x) 'TvsN.dat'  using 1:2 via A,B
...
set output 'TvsN3D.eps'
replot
set output '|epstopdf  TvsN3D.eps'
