/*//------------------------------------------------------------------------------------------------------------
| TEST Tempistiche del metodo di pivoting (trova massimo e poi divide)
|
*///------------------------------------------------------------------------------------------------------------
...
int main(void){
	...
	cout<<"Divido per il Pivot secondo la Gpu Speedy"<<endl;
	cudaEventRecord(T1,0);
	matA.Gpu_Division_Speedy(&pivot_value);
	cudaEventRecord(T2,0);
  	cudaEventSynchronize(T2);
	cudaEventElapsedTime(&diff_time,T1,T2);
	cout << "tempo=" << diff_time<<"\n";
