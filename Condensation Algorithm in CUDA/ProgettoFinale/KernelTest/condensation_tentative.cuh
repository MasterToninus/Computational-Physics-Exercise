/*//---------------------------------------------------------------------------------------------------------
|
|  VERSIONE 1: NESSUN UTILIZZO DI MEMORIA LOCAL SOLO GLOBAL
|
*///---------------------------------------------------------------------------------------------------------
...
__global__ void stepCondensationSimple_v1(float * B,float * A, int numBColumn,int numBRow) 
{
   ...
   while(tid_j<numBColumn){// per ogni macro colonna
	...
	while(tid_i<numBRow){	//per ogni macro riga
		...
		B[tid_i*(numBColumn) + tid_j] = A[tid_i*(numBColumn+1) + tid_j] - A[(numBColumn)*(numBColumn+1) + tid_j]*A[(numBColumn+1)*tid_i + numBColumn];
...
/*//---------------------------------------------------------------------------------------------------------
|
|  VERSIONE 2: NESSUN UTILIZZO DI MEMORIA SHARED SOLO LOCAL
|
*///---------------------------------------------------------------------------------------------------------
//Pensato per essere eseguito con griglia 2d di blocchi 1d
//Scrive la matrice condensato nell'indirizzo B precedentemente allocato! quindi devo avere 2 copie della matrice!

__global__ void stepCondensationSimple_v2(float * B,float * A, int numBColumn,int numBRow) 
{
   ...
   //fase zero, voglio che ogni thread si copia nella chace // i valori dei pivot da utilizzare,  li alloco come local	
   float rigaPivot;
   float colonnaPivot;


  while(tid_j<numBColumn){// per ogni macro colonna
	rigaPivot = A[(numBColumn)*(numBColumn+1) + tid_j];	//copia la riga pivot nella cache,
	//notare numBColum= numAcolum -1!

	while(tid_i<numBRow){	//per ogni macro riga
 		colonnaPivot = A[(numBColumn+1)*tid_i + numBColumn] ;   //ogni thread nel blocco copiera' lo stesso elemento piu' volte, quindi intasamento
		B[tid_i*(numBColumn) + tid_j] = A[tid_i*(numBColumn+1) + tid_j] - rigaPivot*colonnaPivot;
...
/*//---------------------------------------------------------------------------------------------------------
|
|  VERSIONE 3: UTILIZZO DI MEMORIA SHARED Per i Pivot di Riga SOLO LOCAL
|
*///---------------------------------------------------------------------------------------------------------
//Pensato per essere eseguito con griglia 2d di blocchi 1d
//Scrive la matrice condensato nell'indirizzo B precedentemente allocato! quindi devo avere 2 copie della matrice!

__global__ void stepCondensationSimple_v3(float * B,float * A, int numBColumn,int numBRow) 
{
   ...
   // il valore dell'unica colonna pivot da utilizzare lo alloco come local
   float colonnaPivot;
...
/*//---------------------------------------------------------------------------------------------------------
|
|  VERSIONE 5: UTILIZZO DI MEMORIA TEXTURE  2 VOLTE
	il kernel si aspetta che la colonna dei pivot orizzontalizzata E l'intera matrice dei pivot siano precaricate in un array bindato texture
|
*///---------------------------------------------------------------------------------------------------------
//Pensato per essere eseguito con griglia 2d di blocchi 1d
//Scrive la matrice condensato nell'indirizzo B precedentemente allocato! quindi devo avere 2 copie della matrice!
...
__global__ void stepCondensationSimple_v5(float * B,float * A, int numBColumn,int numBRow) 
{
