/*//---------------------------------------------------------------------------------------------------------
|
|  Test di Kernel che utilizzano la memoria texture ( mostrare l'aumento delle prestazioni con una corretta dimensionalizzazione dei thread)
|
*///---------------------------------------------------------------------------------------------------------
...
cout<<"CONDENSAZIONE VERSIONE 5 (2 texture), no ottimizzazione"<<endl; 
...
// ---------
	...
	dim3 block0(DIM_BLOCK );  // 512 x 1 x 1	

	unsigned int block_per_row = DIM_GRID;
	unsigned int block_per_col =  1;
	dim3 grid0( block_per_row,block_per_col);
   ...
   //Kernel
	stepCondensationSimple_v5<<<grid0,block0>>>(B_dev,A_dev, COL-1,ROW-1);		
...
cout<<"CONDENSAZIONE VERSIONE 5 (2 texture), ottimizzazione"<<endl; 
	//registro il primo tempo
	cudaEventRecord(T1, 0);
...
// ---------
	...
	dim3 block1(MAX_DIM_BLOCK );  // 512 x 1 x 1	
...
// ---------
        ...
        dim3 block4(MAX_DIM_BLOCK );  // 512 x 1 x 1    
